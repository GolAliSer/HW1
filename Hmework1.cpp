...
void KvYravn(double a, double b1, double c) { //Функция KvYravn вычисляет коэффициенты квадратного уравнения
    ...
    if (discr > 0) { //условие "if", которое проверяет, что дискриминант больше нуля
        ...
        std::cout << " D > 0, dva deistvitelnyh korny " << r1 << " и " << r2 << std::endl; //вывод значения корней на экран 
    }
    else 
...
int main()
{
    ...
    double b2 = 0.0; // cоздается переменная b2 типа double и инициализируется значением 0.0
    ...
    /* Task 5*/
    std::cout << "Vvedite coordinaty x: ";  //Выводится сообщение "Vvedite coordinaty x: " 
    double x = 0.0;
    ...
    std::cin >> p >> f; //введение переменных с консоли
    ...
    std::cin >> a; //введение переменной с консоли
    std::cout << "b = ";
    std::cin >> b1; //введение переменной с консоли
    std::cout << "c = ";
    ...
    std::cin >> pr; //вводим с консоли переменную
    ...
    //Условие, которое определяет, что больше произведение йифп числа или число b
    if (pro > b) {
